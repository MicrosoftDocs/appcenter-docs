...
You can also optionally add **one binary** and **one text** attachment to a handled error report. Please be sure the attachment file is **not** named `minidump.dmp` as that name is reserved for minidump files. Pass the attachments as an array of `ErrorAttachmentLog` objects as shown in the example below.
