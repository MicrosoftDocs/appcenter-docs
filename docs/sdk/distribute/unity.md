...
App Center Distribution enables your users/testers to install new versions of an app once you distribute it via App Center. With the SDK enabled, when you publish an update the SDK presents an update dialog to the users to either download or postpone installation of the new version. Once they choose to update, the SDK will start to update your application.
...
Please follow the [Unity getting started](~/sdk/getting-started/unity.md) docs if you haven't set up and started the SDK in your application, yet. Make sure to import the App Center Distribute package. Its name should be in the format **AppCenterDistribute-v{version}.unitypackage**
...
You can enable and disable App Center Distribute at runtime. If you disable it, the SDK will not provide any in-app update functionality but you can still use the Distribute service in the App Center portal.
...
2. Once you integrate the SDK, build a release version of your app and upload it to App Center, users in that distribution group are notified for the new release via an email.
