...
For example, consider a use-case where the initial installed binary version is 1.0.1 with the label(codepush label) v1, then the binary version was updated to 1.0.2 after a change. When the code-push update check is triggered, the installed app (1.0.1) will ignore the update targeting version 1.0.2. You can use `handleBinaryVersionMismatchCallback` to provide a hook to handle such situations.
