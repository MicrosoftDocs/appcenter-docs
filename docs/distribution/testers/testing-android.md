...
> [!NOTE]
> Visual Studio App Center is no longer available in the Google Play store. Unfortunately, Google Play store considers the in-app update code as malicious behavior even if it isn’t used at runtime. Due to Google Play store policies, there are no current plans to bring the app back to the Google Play store. The same set of functionality is still available via he App Center install portal. 
